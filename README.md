# Legend Barber Queue App React Native

## Setup Commands

### App Create Commands

npx react-native init LegendsBarberQueue --template react-native-template-typescript

<!-- gem install bundler
update Gemfile content
run bundle update
add the fololowing files in the fastlane dir. Appfile and Fastfile -->

### get the google service account

reuse

### generate sigining

keytool -genkey -v -keystore legendBQrelease.keystore -alias legendBQ -keyalg RSA -keysize 2048 -validity 10000
mkdir lgends-barber-queue && cd lgends-barber-queue && keytool -genkey -v -keystore release.keystore -alias legendBQ -keyalg RSA -keysize 2048 -validity 10000

### fastlane

cd into android folder and run

cd into fastlane directory
add service_account.json
go to git ignore and ignore the file
run fastlane run validate_play_store_json_key json_key:service_account.json

Retrieve the version number of the application in the package.json file that will define the version number of the Android App Bundle.
run
fastlane add_plugin load_json

run fastlane add_plugin versioning_android
